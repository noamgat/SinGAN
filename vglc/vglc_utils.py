    ...
    for levelidx in range(len(game)):
        ...
        def downscale_image(source_image_2d):
            ...
            for ii in range(0, opt.stop_scale + 1, 1):
                ...
                from vglc import resizer
                target_size = np.uint32(np.ceil((h * scale, w * scale)))
                sample2 = resizer.imresize(source_image_2d, output_shape=target_size, kernel='box')
...
def save_sc2_image(sc2_image: np.ndarray, filename: str):
    from PIL import Image
    uint_image = convert_sc2_image_to_image(sc2_image)
    im = Image.fromarray(uint_image)
    im.save(filename)
    # uint_image_reorder = np.moveaxis(uint_image, [0, 1, 2], [1, 2, 0])
    # uint_image_list = list(uint_image_reorder)
    # save_numpy_rgb_uint8_images(uint_image_list, filename)


def stack_color_channels(uint_image: np.ndarray) -> np.ndarray:
    uint_image_reorder = np.moveaxis(uint_image, [0, 1, 2], [1, 2, 0])
    uint_image_list = list(uint_image_reorder)
    concanenated_image = np.concatenate(uint_image_list, axis=0)
    return concanenated_image

def convert_sc2_image_to_image(sc2_image: np.ndarray) -> np.ndarray:
    # Stack em up
    uint_image = (sc2_image * 255).astype(np.uint8)
    concanenated_image = stack_color_channels(uint_image)
    return concanenated_image
