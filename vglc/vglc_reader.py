...
class VGLCGameData:
    def __init__(self, json_path, levels_dir):
        ...
        if json_path == 'sc2':
            self.is_sc2_mode = True
            from pathlib import Path
            for filename in Path(levels_dir).rglob('*.npz'):
                level_map = np.load(filename)['map']
                self.levels_contents.append(level_map)
        else:
            self.is_sc2_mode = False
