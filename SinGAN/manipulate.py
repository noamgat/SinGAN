...
def generate_gif(Gs,Zs,reals,NoiseAmp,opt,alpha=0.1,beta=0.9,start_scale=2,fps=10):
    ...
    for G,Z_opt,noise_amp,real in zip(Gs,Zs,NoiseAmp,reals):
        ...
        if count == 0:
            ...
            z_rand = z_rand.expand(1,opt.nc_im,Z_opt.shape[2],Z_opt.shape[3])
    ...
    for G,Z_opt,noise_amp in zip(Gs,Zs,NoiseAmp):
        ...
        for i in range(0,num_samples,1):
            if n == 0:
                ...
                z_curr = z_curr.expand(1,opt.nc_im,z_curr.shape[2],z_curr.shape[3])
                z_curr = m(z_curr)
            else:
            ...
            if n == len(reals)-1:
                ...
                if (opt.mode != "harmonization") & (opt.mode != "editing") & (opt.mode != "SR") & (opt.mode != "paint2image"):
                    plt.imsave('%s/%d.png' % (dir2save, i), functions.convert_image_np(I_curr.detach(), opt), vmin=0,vmax=1)
